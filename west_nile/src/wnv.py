...
def xtrain():
    ...
    Xtest = assemble_X(testing, weather) 
    sum_wneg=0;sum_wpos=0;
...
def submit(net, mean, std):
    ...
    fpred=0.3*xtrain()+0.7*predictions    

    out = csv.writer(open("west_nile_lasagne.csv", "w"))
    out.writerow(["Id","WnvPresent"])
    for row, p in zip(testing, fpred):
