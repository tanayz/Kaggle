...
############################################################################################
x<-dtrain[2:31]
 nnetAv <- avNNet(x1, dtrain[[33]],size = 5,decay = 0.01,repeats = 5,linout = TRUE,
...
#                   cost = 2^c(-8,-4,-2,0))
frft <- train(x,label,method = "GFS.LT.RS",preProc = c("center", "scale"),
                            trControl = trainControl(method = "cv"))

ypre0= xgb.predict(bst, xgmat) # AMS = 4.331712  => 3.896457
ypre1=predict(nnetAv, newdata=x) # AMS = 2.944339 1=>
ypre2=predict(larft, newdata=x) # AMS = 1.556337
ypre3=predict(fbft, newdata=x) # AMS = 1.561891
ypre4=predict(rffit,newdata=x) # AMS = 35.39238
ypre5=predict(frft,newdata=x) # AMS = 1.33
ypre=(ypre5)
rorder <- rank(ypre, ties.method="first")
