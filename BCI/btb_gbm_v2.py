...
train.to_csv('data/train_cz.csv',ignore_index=True)
...
test.to_csv('data/test_cz.csv',ignore_index=True)

print 'training GBM'

gbm = ens.GradientBoostingClassifier(n_estimators=500,learning_rate=0.01, max_features=0.25)
gbm.fit(train, train_labels.values[:,1].ravel())
preds = gbm.predict_proba(test)
preds = preds[:,1]
submission['Prediction'] = preds
submission.to_csv('data/gbm_benchmark.csv',index=False)
print 'Done'